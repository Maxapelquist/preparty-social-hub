...
-- First, we need to clean up existing duplicate friendships before adding the unique constraint

-- Add pair_key column to friends table if it doesn't exist
ALTER TABLE public.friends 
ADD COLUMN IF NOT EXISTS pair_key text;

-- Create function to generate pair key
...
-- Update all existing records to have pair_key
UPDATE public.friends 
SET pair_key = generate_friendship_pair_key(user_id, friend_id)
WHERE pair_key IS NULL;

-- Remove duplicate friendships - keep only the earliest one for each pair
DELETE FROM public.friends f1 
WHERE f1.id IN (
  SELECT f1.id
  FROM public.friends f1
  INNER JOIN public.friends f2 ON f1.pair_key = f2.pair_key
  WHERE f1.id > f2.id
);
...
DROP TRIGGER IF EXISTS trigger_set_friendship_pair_key ON public.friends;
